  ...
  return (
    <section className="container-shell py-10">
      <div className="mx-auto max-w-2xl glass-panel rounded-[2rem] p-8 sm:p-10">
        ...
        <p className="mt-3 text-[#334155]">Talaba yoki o‘qituvchi sifatida profil yarating. Administrator akkauntlari boshqaruv orqali yaratiladi.</p>
        <form onSubmit={handleSubmit} className="mt-8 grid gap-4 md:grid-cols-2">
          <input value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} placeholder="To‘liq ism" className="input-shell md:col-span-2" required />
          ...
          <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="Elektron pochta" className="input-shell" required />
          ...
          <div className="rounded-2xl border border-dashed border-[#dbe7f3] bg-[#f8fbff] px-4 py-3 text-sm text-[#64748b]">
            Rollarga asoslangan autentifikatsiya backend orqali boshqariladi.
          </div>
        ...
        <p className="mt-5 text-sm text-[#64748b]">
          Akkount bormi? <Link to="/login" className="font-semibold text-[#2563eb]">Kiring</Link>
        </p>
      </div>
    </section>
  );
